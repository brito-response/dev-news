// import { ActionButtonsBar } from "@/components/ActionButtonsBar";
  // import { User } from "@/utils/models/users";
  import { FileCogIcon, FileText, TableConfigIcon } from "lucide-react";
  import Link from "next/link";

  type User = {};
  async function getusers(): Promise<User[]> {
    const response = await fetch(`${process.env.NEXT_BACKEND_URL}/users`, {
      cache: "no-store",
    });
    if (!response.ok) return [];
    const users: User[] = await response.json();
    return users;
  }

  export default async function UsersPage() {
    const users: User[] = await getusers();

    return (
      <div className="w-full min-h-screen bg-[--bg-section-100] p-10 transition-colors duration-500">
        <div className="max-w-6xl mx-auto">
          {/* Header */}
          <header className="flex items-center justify-between mb-10">
            <div className="flex items-center gap-3">
              <div className="p-2 rounded-xl bg-primary/10 text-primary">
                <FileText className="w-6 h-6" />
              </div>
              <div>
                <h1 className="text-3xl font-semibold">Posts</h1>
                <p className="text-muted-foreground">
                  Lista de posts publicados na aplicação
                </p>
              </div>
            </div>
          </header>

          {/* Users list */}
          <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {users.map((user: User) => (
              <article key={user.userId} className="rounded-2xl bg-background p-6 shadow-sm hover:shadow-md transition-shadow flex flex-col justify-between">
                <div className="flex items-start gap-3">
                  <div className="p-2 w-full bg-muted text-muted-foreground ">
                    <h2 className="font-semibold text-lg leading-snug">
                      {user.title}
                    </h2>
                    <div className="flex items-center justify-between">
                      <FileText className="w-5 h-5" />
                      <Link href={`/users/${user.userId}/config`} className="cursor-pointer p-2 hover:bg-amber-400 rounded-2xl">
                        <FileCogIcon className="w-5 h-5" />
                      </Link>

                    </div>

                  </div>
                </div>
                // <ActionButtonsBar linkToEdit={`/users/${user.userId}/edit`} linkToDelete={`/user/${user.userId}/delete`} />
              </article>
            ))}
          </section>

          {/* Empty state */}
          {users.length === 0 && (
            <div className="mt-20 text-center text-muted-foreground">
              <FileText className="w-10 h-10 mx-auto mb-3 opacity-50" />
              <p>Nenhum user encontrado</p>
            </div>
          )}
        </div>
      </div>
    );
  };
