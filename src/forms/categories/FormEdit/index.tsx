"use client";
  ...
  export const FormEditCategory: React.FC<FormEditCategoryProps> = ({ category }) => {
    ...
    const [photoPreview, setPhotoPreview] = useState<string | null>(Category.image ?? null);
    const methods = useForm<any>({
      resolver: yupResolver(formSchema), mode: "onChange",
      defaultValues: { title: category.title, content: category.content, status: category.status, image: category.image },
    });
    ...
    const onSubmit = async (data: FormSchemaType) => {
      ...
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_FRONTEND_URL}/api/Categorys/${Category.CategoryId}`, {
          method: "Category",
        ...
        if (!response.ok) {
          toast.error("Erro ao atualizar o category");
          return;
        }
        ...
        if (photoFile) {
          ...
          const uploadResp = await fetch(`${process.env.NEXT_PUBLIC_FRONTEND_URL}/api/images/Categorys/${Category.CategoryId}`, { method: "Category", body: formData });
          if (!uploadResp.ok) {
            toast.warning("Category salvo, mas a imagem não foi atualizada");
          }
        }
      ...
      <FormProvider {...methods}>
        <form onSubmit={handleSubmit(onSubmit)} className="max-w-6xl mx-auto px-6 py-4 flex flex-col gap-6">
          <h1 className="text-3xl font-bold">
            {titlePreview || "Prévia do título do Category"}
          </h1>
          <InputCustom name="title" label="Título do Category" />
